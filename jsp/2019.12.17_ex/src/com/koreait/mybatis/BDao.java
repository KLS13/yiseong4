...
public class BDao {
	...
	//회원가입
	public int getInsertJoin(JDto jdto) {
		SqlSession sqlSession = factory.openSession(false);	// sqlSession 생성
		int result = sqlSession.insert("insert_join", jdto);
		if ( result > 0 ) {
			sqlSession.commit();
		}
		sqlSession.close();
	...
	public List<JDto> getLogin(JDto jdto) {
		SqlSession sqlSession = factory.openSession();
		List<JDto> list = sqlSession.selectList("list_login",jdto);
		sqlSession.close();
		return list;
		// return sqlSession.selectList("list_bbs");
	}
